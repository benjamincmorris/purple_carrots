...
```{r}
#testing
tmp <- readLines("~/Downloads/KB1RE00D.cha")
...
tmp_df %>%
  ...
  mutate(particpant = strsplit(line, "|", fixed = T)[[1]][3])
```


```{r}
#get all corpus .cha files
file_list <- fs::dir_ls(path = "~/Downloads/CABNC/", recurse = TRUE, type = "file", glob = "*.cha")
...
final_CABNC_transcript_list_txt <- ages %>%
  ...
  distinct(directory) %>%
  rowwise() %>%
  mutate(directory_clean = str_split(directory, "/Users/benmorris/Downloads/CABNC")[[1]][2]) %>%
  mutate(directory_clean = str_split(directory_clean, ".cha")[[1]][1]) %>%
  mutate(directory_clean = str_glue(directory_clean, ".txt")) %>%
  mutate(directory_clean = str_glue("/Users/benmorris/Documents/corpora/CABNC-Corpus/cabnc_data/text", directory_clean)) %>%
...
file_list <- fs::dir_ls(path = "~/Documents/corpora/CABNC-Corpus/cabnc_data/text/", recurse = TRUE, type = "file", glob = "*.txt")
...
write.table(file = cabnc_transcripts, "~/Downloads/cabnc_adult_utterances.csv")
```
