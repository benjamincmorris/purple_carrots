...
abstract: >
    Language provides children a powerful source of information about the world. However, language does not perfectly reflect the world: what is typical may often go unremarked. For instance, adults rarely describe the color of an orange carrot, as world knowledge makes this description redundant. Given children’s nascent world knowledge, does parents’ speech to children follow this pattern? From a longitudinal corpus of parent-child communication [@goldin2014] between 14–58 months, we extracted usage data for 637 high-frequency concrete nouns and co-occurring adjectives. Independent raters coded the typicality of 1,901 unique adjective–noun pairs on a 7-point Likert scale. We find that parent language highlights the atypical features of things, across all child ages. Parents were reliably more likely to use typical descriptors when talking to younger compared to older children. Using vector space models trained on our corpus and on Wikipedia, we show that rich typicality information remains difficult to extract from language alone.  
...
Still, there is reason to believe that some semantic features may be harder to learn from language than these data suggest. This is because people rarely use language merely to provide running comentary on the world around them; instead, we use language to talk about things that diverge from our expectations or those of our conversational partner [@grice1975]. People largely avoid being over- or under-informative when they speak, leaving what should be obvious unstated and mentioning what is notable. Referential communication tasks in the lab largely bolster this claim, finding that speakers provide roughly as much description as is necessary to identify an object [@mangold_1988]. In particular, speakers are informative with respect to both the referential context and the typical features of the referent [@mitchell_2013; @westerbeek2015; @rubio-fernandez_2016]. Speakers tend to refer to an object that is typical of its category with a bare noun (e.g., calling an orange carrot "a carrot"), but often specify when an object has an atypical feature (e.g, referring to a purple carrot as such). Given these communicative pressures, naturalistic language statistics may provide surprisingly little evidence about what is typical [@willits2008]. 
...
On the other hand, parents may speak to children far differently from the way they speak to other adults. Parental speech may reflect typical features of the world more veridically, or even emphasize typical features in order to teach children about the world. Parents alter their speech to children along a number of structural dimensions, using simpler syntax and more reduplications (Snow, 1972). Their use of description may reflect similar alignment to children's growing knowledge.
