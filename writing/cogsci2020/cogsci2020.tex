...
the the object to which they are referring (Rubio-Fernández, 2016;
Westerbeek, Koolen, \& Maes, 2015). People tend to refer to an object
that is typical of its category with a bare noun (e.g., calling an
orange carrot ``a carrot''), but often specify when an object has an
atypical feature (e.g, ``a purple carrot''). Given these communicative
pressures, naturalistic language statistics may provide surprisingly
little evidence about what is typical (Willits, Sussman, \& Amato,
2008).
...
the 79 nouns. Table \ref{tab:pairs_tab} gves the six cases in which
word2vec similarities are worst at predicting human typicality
judgments, judging the low-typicality adjective to be more similar than
the high-typicality.
...
apple & red & orange \\ 
  ...
  elephant & fat & pink \\ 
   \hline
\end{tabular}
\caption{This table shows the six cases in which word2vec similarities are worst at predicting human typicality judgments, judging the low-typicality adjective to be more similar than the high-typicality adjective.} 
\label{tab:pairs_tab}
\end{table}
...
that children expect interlocutors to comment on what is new by age 2
(Akhtar, Carpenter, \& Tomasello, 1996), and may provide roughly
infomrative speech themselves by age 5-6 (Nadig \& Sedivy, 2002).
Indeed, such findings are consistent with our data that even young
children also largely choose to describe atypical features. Though this
effect can be explained by simpler means such as salience or mimicry, it
