...
abstract: |
  How do children learn the typical features of things in the world? For many objects, this information must come from the language they hear. However, language does not necessarily reflect the world veridically. If the language children hear selectively picks out the atypical features of things (e.g., "purple carrot") rather than the typical features of things (e.g., "orange carrot"), learning about the world from language is less straightforward. Here, we test whether the language children hear from parents, as well as everyday conversation among adults, overrepresents atypical features. To do so, we examined the typicality of features in adjective-noun pairs produced by parents in a large, longitudinal corpus of parent-child interaction, as well as a comparison set of adjective-noun pairs from adult-adult speech. Across over 6,000 unique adjective–noun pairs, we found that parents speaking to children—like adults speaking to other adults—predominantly use adjectives to mark atypical features of things. We also found that parents of very young children comment on typical features slightly more often than parents of older children. Thus, language is structured to emphasize what is atypical—so how can one learn about what things are typically like from language? We also show that distributional semantics models that use word co-occurrence to derive word meaning (word2vec) do not capture the typicality of adjective–noun pairs well. A much more sophisticated language model (GPT-3) does capture the typicality of adjective noun pairs well; though this model has input unlike what children have access to, it provides useful bounds on the typicality information learnable from applying simple training objectives to language alone. Taken together, this work raises new foundational questions about how children manage to learn so much from language that does not directly reflect the world, but selectively picks out remarkable facets of it.
