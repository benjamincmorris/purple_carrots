...
filter_concrete <- function(pairs) {
  ...
  filter(adj %in% most_concrete$Word &
    ...
    select(-adj_stem, -noun_stem)
}
```

# Compare all UDPipe pairs to our original set of pairs from alll of LDP

```{r}
udp_parse <- read_csv(here("data/ldp_parent_original_parsed_pairs_allmods.csv")) %>%
...
#pairs in both sets
inner_join(ldp_pairs, udp_pairs, by = c("adj", "noun"))

# udp parsed pairs that aren't in our original ldp set
anti_join(udp_pairs, ldp_pairs, by = c("adj", "noun"))
...
anti_join(udp_pairs, ldp_pairs, by = c("noun")) %>% count(noun) %>% arrange(desc(n))
