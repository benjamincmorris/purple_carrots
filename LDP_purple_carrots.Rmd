...
```
###quick descriptives:
- 9,174 distinct nouns used across LDP (probably some more data cleaning to do...)
...
full_turk_counts %>% group_by(adj, noun) %>% summarize(n=sum(n)) %>% arrange(desc(n)) %>% select(adj, noun, n)
...
tmp <- prenominals %>% filter(prenominal) %>% ungroup() %>%
  select(subject.x, session.x, noun, adj, prenominal, x1, x2, x3, x4, x5, mean_typ)

write.csv(tmp, "prenominal_counts.csv")

prototypicals <- prenominals %>% 
  mutate(typicality=mean_typ) %>%
  mutate(howTyp = if_else(typicality>=5, T, F)) %>%
  group_by(session.x, prenominal, howTyp) %>%
  summarize(sum=n()) %>%
  mutate(prop=sum/sum(sum)) %>%
