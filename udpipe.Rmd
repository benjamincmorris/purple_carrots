...
get_pairs <- function(filename) {
  ...
  adjs <- adj_parses %>%
    filter(upos == "ADJ") %>%
    select(doc_id, sentence, chat, token_id, token, head_token_id, lemma, upos,
           dep_rel) %>%
  ...
  nouns_setup <- adj_parses %>%
    select(doc_id, sentence, chat, token_id, token, lemma, upos, dep_rel,
           head_token_id)
  ...
  noncompounds <- nouns_setup %>%
    filter(dep_rel != "compound") %>%
    select(doc_id, sentence, chat, token, lemma) %>%
    rename(noun_token = token, noun_lemma = lemma)
    
 # possible_pairs <-  adjs %>%
 #    bind_rows(compounds) %>%
 #    select(doc_id, sentence, chat, adj_token, adj_lemma) %>%
 #    left_join(noncompounds, by = c("doc_id", "sentence", "chat"))
  
  write_csv(pairs, here(glue("data/{filename}_parsed_pairs_bothmods.csv")))
