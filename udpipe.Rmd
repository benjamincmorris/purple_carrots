...
get_pairs <- function(filename) {
  ...
  root_adjs <- adj_parses %>%
    ...
    rename(noun_token_id = head_token_id) %>%
    mutate(noun_token = "ROOT", noun_lemma = "ROOT", prenominal = FALSE,
           noun_token_id = 0,
  ...
  pairs <- adjs %>%
    ...
    bind_rows(root_adjs) %>% # add ROOT adjs
    filter(!adj_token == "um")
  ...
  write_csv(pairs, here(glue("data/{filename}_parsed_pairs_allmods.csv")))
